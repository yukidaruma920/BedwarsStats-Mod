...
public class HypixelApiHandler {
    ...
    private static String formatStats(JsonObject player, BedwarsStatsConfig.BedwarsMode mode) {
        ...
        String rankPrefixText = config.showRankPrefix ? getRankPrefix(player) : "";

        if (!player.has("stats") || player.get("stats").isJsonNull() || !player.getAsJsonObject("stats").has("Bedwars")) {
            return rankPrefixText + rankColor + username + "§7: No Bedwars stats found.";
        }
        ...
        String[] displayOrder = config.displayOrder.split(",");
        for (int i = 0; i < displayOrder.length; i++) {
            try {
                BedwarsStatsConfig.Stat stat = BedwarsStatsConfig.Stat.valueOf(displayOrder[i].trim().toUpperCase());
                switch (stat) {
                    case WINS: statsBuilder.append(String.format("Wins %s%s§r", getWinsColor(wins), formatNumber(wins))); break;
                    case FINALS: statsBuilder.append(String.format("Finals %s%s§r", getFinalsColor(finalKills), formatNumber(finalKills))); break;
                    case FKDR: statsBuilder.append(String.format("FKDR %s%.2f§r", getFkdrColor(fkdr), fkdr)); break;
                    case WLR: statsBuilder.append(String.format("WLR %s%.2f§r", getWlrColor(wlr), wlr)); break;
                    case WS: statsBuilder.append(String.format("WS %s%d§r", getWsColor(winstreak), winstreak)); break;
                }
                if (i < displayOrder.length - 1) statsBuilder.append(" | ");
            } catch (IllegalArgumentException e) { /* Ignore invalid stat names */ }
        }

        return String.format("%s %s%s%s§r §7[%s]§r: %s",
                prestige, rankPrefixText, rankColor, username, mode.getDisplayName(), statsBuilder.toString());
    }
    ...
    private static String getFkdrColor(double fkdr) { if (fkdr >= 20) return "§5"; if (fkdr >= 15) return "§d"; if (fkdr >= 10) return "§4"; if (fkdr >= 8)  return "§c"; if (fkdr >= 6)  return "§6"; if (fkdr >= 4)  return "§e"; if (fkdr >= 2)  return "§2"; if (fkdr >= 1)  return "§a"; return "§7"; }
    private static String getWlrColor(double wlr) { if (wlr >= 10) return "§5"; if (wlr >= 8)  return "§d"; if (wlr >= 6)  return "§4"; if (wlr >= 5)  return "§c"; if (wlr >= 4)  return "§6"; if (wlr >= 3)  return "§e"; if (wlr >= 2)  return "§2"; if (wlr >= 1)  return "§a"; return "§7"; }
    private static String getWinsColor(int wins) { if (wins >= 50000) return "§5"; if (wins >= 25000) return "§d"; if (wins >= 10000) return "§4"; if (wins >= 5000) return "§c"; if (wins >= 2500) return "§6"; if (wins >= 1000) return "§e"; if (wins >= 500) return "§2"; if (wins >= 250) return "§a"; return "§7"; }
    private static String getFinalsColor(int finals) { if (finals >= 100000) return "§5"; if (finals >= 50000) return "§d"; if (finals >= 25000) return "§4"; if (finals >= 10000) return "§c"; if (finals >= 5000) return "§6"; if (finals >= 2500) return "§e"; if (finals >= 1000) return "§2"; if (finals >= 500) return "§a"; return "§7"; }
    private static String getWsColor(int ws) { if (ws >= 100) return "§4"; if (ws >= 50) return "§6"; if (ws >= 25) return "§e"; if (ws >= 10) return "§a"; if (ws >= 5) return "§2"; return "§7"; }
